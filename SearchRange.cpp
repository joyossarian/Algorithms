...
vector<int> searchRange(int A[], int n, int target) {
	...
	vector<int> result;
	int firstTrial = binarySearch(A, 0, n - 1, target);
	if (firstTrial == -1)
	{
		result.push_back(-1);
		result.push_back(-1);
	...
	else
	{
		...
		while ((lastUpper = binarySearch(A, upperStart, n - 1, target)) != -1)
		{
		...
		while ((lastLower = binarySearch(A, 0, lowerStart, target)) != -1)
		{
			prevlastLower = lastLower;
			lowerStart = prevlastLower - 1;
		}
		result.push_back(prevlastLower);
		result.push_back(prevLastUpper);
	}
	return result;
}
int main()
